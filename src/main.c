...
/* Private typedef -----------------------------------------------------------*/
/* Private define ------------------------------------------------------------*/
/* Use DEBUG if you want to see in the Terminal I/O the current task*/
#define DEBUG
/* Private macro -------------------------------------------------------------*/
/* Private variables ---------------------------------------------------------*/
...
void task_LED_OFF(void *i_unused);
void task_LED_ON(void *i_unused);
/* Private functions ---------------------------------------------------------*/

/**
 * @brief   Main program. We will create two tasks with FreeRTOS in order to make
 * the LED blink.
 * @param  None
 * @retval None
 */
int main(void)
{
	...
	/*Now we create the two tasks. One will turn on the LED, the other will turn it OFF.
	 * They both have the same priority so the OS will switch between their execution*/
	xTaskCreate(task_LED_OFF,"LED OFF TASK",100,NULL,
			1,NULL);
	xTaskCreate(task_LED_ON,"LED ON TASK",100,NULL,
			1,NULL);
	/* Start the scheduler. */
	vTaskStartScheduler();
...
/**
 * @brief Turns off the LED and wait 500ms
 * @param i_unused unused
 */
void task_LED_OFF(void *i_unused)
{
	for(;;)
...
/**
 * @brief Turns on the LED and wait
 * @param i_unused unused
 */
void task_LED_ON(void *i_unused)
{
